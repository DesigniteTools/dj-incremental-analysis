...
def download_artifact(artifacts, designite_output_old, token):
    ...
    filtered_artifacts = list(filter(lambda artifact: artifact["name"] == designite_output_old, artifacts))

    if not filtered_artifacts or len(filtered_artifacts) >1:
        print(f"No artifacts found for this repository or multiple artifacts found - {designite_output_old}. Please check the artifact name.")
        return False
